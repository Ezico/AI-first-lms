...
export async function registerUser(formData: FormData) {
  ...
  try {
    ...
    // Create user
    await executeQuery("INSERT INTO users (name, email, password, role) VALUES ($1, $2, $3, $4)", [
      name,
      email,
      hashedPassword,
      "user",
...
export async function updateUserProfile(formData: FormData) {
  try {
    ...
    if (email !== user.email) {
      const existingUsers = await executeQuery("SELECT * FROM users WHERE email = $1 AND id != $2", [email, user.id])
    ...
    await executeQuery("UPDATE users SET name = $1, email = $2, image = $3 WHERE id = $4", [
      name,
      email,
      imageUrl,
      user.id,
...
export async function updateUserPassword(formData: FormData) {
  try {
    ...
    const users = await executeQuery("SELECT * FROM users WHERE id = $1", [user.id])
    ...
    await executeQuery("UPDATE users SET password = $1 WHERE id = $2", [hashedPassword, user.id])
